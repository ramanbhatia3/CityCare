...
const WhatsAppShareButton = ({ issue }) => {
  ...
  return (
    <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-4 py-2 bg-green-500 text-white font-semibold rounded-lg hover:bg-green-600 transition-colors duration-200">
      <WhatsAppIcon />
      Share
    </a>
  );
...
const AuthorityUpdateForm = ({ issueId, userInfo, onUpdateAdded }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!text.trim()) { alert('Update text cannot be empty.'); return; }
    ...
    try {
      ...
      const { data } = await axios.post(`${apiUrl}/api/issues/${issueId}/update`, formData, config);
      onUpdateAdded(data.authorityUpdates);
      setText('');
      setImage(null);
      e.target.reset();
    } catch (error) {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mt-2 flex items-center justify-between">
          <label className="flex items-center text-sm text-gray-600 cursor-pointer">
            ...
            <span className={image ? 'text-green-600 font-semibold' : ''}>{image ? 'Image selected!' : 'Attach Photo (Optional)'}</span>
            <input type="file" className="hidden" accept="image/*" onChange={(e) => setImage(e.target.files[0])} />
          </label>
          <button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 flex items-center disabled:bg-indigo-400">
            <Send className="w-4 h-4 mr-2" />{isSubmitting ? 'Posting...' : 'Post'}
          </button>
        </div>
      </form>
    </div>
...
export default function IssueDetailPage({ issueId, navigateTo, userInfo }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000';
        const issueRes = await axios.get(`${apiUrl}/api/issues/${issueId}`);
        setIssue(issueRes.data);
        const commentsRes = await axios.get(`${apiUrl}/api/comments/${issueId}`);
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000';
      const { data } = await axios.post(`${apiUrl}/api/comments/${issueId}`, { text: newComment }, config);
      
      // Re-fetch comments to get updated user data
      const commentsRes = await axios.get(`${apiUrl}/api/comments/${issueId}`);
      setComments(commentsRes.data);
      setNewComment('');
    } catch (err) {
